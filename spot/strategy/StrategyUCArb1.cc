...
int StrategyUCArb1::maker_action() {
    ...
    bool bid_continue = true; //# best_price buy 鍙栨渶楂� sell 鍙栨渶浣�
    if (IS_DOUBLE_GREATER(po_best_price(buyPriceMap2, 0), sy2.bid_place_price * (1 + batch_sensitivity))) {  // 澶撮儴鎾ゅ崟 閮ㄥ垎鎾ゅ崟锛屽彧鎾ゆ棤鍒╁彲鍥惧崟
    ...
    if (getBuyPendingLen(sy2) != 0)
        bid_continue = false; //#鏈夋寕鍗�

    bool ask_continue = true;
    // # ask order
    if (IS_DOUBLE_LESS(po_best_price(sellPriceMap2, 1), sy2.ask_place_price * (1 - batch_sensitivity))) { //  # 澶撮儴鎾ゅ崟
    ...
    if (bid_continue && !(!buy_open_flag && IS_DOUBLE_GREATER(step_thresh, 0) && IS_DOUBLE_ZERO(sy2.strategyInstrument->position().getNetPosition()))) {
        ...
        if (IS_DOUBLE_GREATER(po_best_price(buyPriceMap2, 0), temp_bid1 + epsilon) && current_level < order_level) {  //# 灏鹃儴鎸傚崟
            auto it = buyPriceMap2->begin();
            double last_order_px = it->first; //# bid: 楂�->浣�   ask锛� 浣�->楂�
        ...
        else if (current_level > order_level + level_tolerance) { // # 灏鹃儴鎾ゅ崟
        ...
        else if (current_level != 0 && IS_DOUBLE_LESS(po_best_price(buyPriceMap2, 0), worst_bid - epsilon)) { // 鎾ゅ崟
            for (auto it = buyPriceMap2->begin(); it != buyPriceMap2->end(); it++) {
                for (auto& iter : it->second->OrderList) {
        ...
        else if (IS_DOUBLE_LESS(po_best_price(buyPriceMap2, 0), temp_bid1 - epsilon)) { //# 澶撮儴鎸傚崟
            if (current_level == 0) {
    ...
    if (ask_continue && !(!sell_open_flag && IS_DOUBLE_LESS(step_thresh, 0) && IS_DOUBLE_ZERO(sy2.strategyInstrument->position().getNetPosition()))) {
        ...
        if (IS_DOUBLE_LESS(po_best_price(sellPriceMap2, 1), temp_ask1 - epsilon) && current_level < order_level) { //# 灏鹃儴鎸傚崟
        ...
        else if (current_level != 0 && po_best_price(sellPriceMap2, 1) > worst_ask + epsilon) {  //# 鎾ゅ崟
            for(auto it = sellPriceMap2->rbegin(); it != sellPriceMap2->rend(); it++) {
                for(auto& iter : it->second->OrderList) {
        ...
        else if (IS_DOUBLE_GREATER(po_best_price(sellPriceMap2, 1), temp_ask1 + epsilon)) {  //# 澶撮儴鎸傚崟
            if (current_level == 0) {
                for (int i = 0; i < order_level; i++) {
                    SetOrderOptions order;
                    order.orderType = ORDERTYPE_LIMIT_CROSS; // ?
...
void StrategyUCArb1::update_thresh() 
{
    ...
    if (IS_DOUBLE_GREATER(abs(sy2.real_pos) * sy2.mid_price, max_position_limit)) {  //# 浠撲綅cap
...
try {
    ...
    // to do log side 鏄拱鐨勮瘽鎵撳嵃 ob ask(娌℃敼涔嬪墠)锛� 鍙嶄箣ob buy; exch(鍙敹bybit鐨則rade), 
    ...
    // to do log side 鏄拱鐨勮瘽鎵撳嵃 ob ask锛堟敼浜嗕箣鍚庯級锛� 鍙嶄箣ob buy; exch(鍙敹bybit鐨則rade), 
