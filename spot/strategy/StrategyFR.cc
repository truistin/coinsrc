...
double StrategyFR::calc_future_uniMMR(sy_info& info, double qty)
{
    ...
    if (IS_DOUBLE_GREATER(IM, sum_equity)) {
        LOG_INFO << "锟街伙拷+锟斤拷约锟侥筹拷始锟斤拷证锟斤拷 > 锟斤拷效锟斤拷证锟金，诧拷锟斤拷锟斤拷锟铰卄1锟�1锟�77: " << IM << ", sum_equity: " << sum_equity;
        return 0;
    }
...
double StrategyFR::calc_predict_equity(sy_info& info, order_fr& order, double price_cent)
{
    ...
    if ((SPOT == info.type && info.long_short_flag == 0) || (SWAP == info.type && info.long_short_flag == 1)) { // 锟街伙拷锟斤拷锟斤拷锟�1锟�1锟�77 锟斤拷约锟斤拷锟斤拷
    ...
    } else { // 锟街伙拷锟斤拷锟秸★拷1锟�1锟�77 锟斤拷约锟斤拷锟斤拷
...
double StrategyFR::calc_predict_mm(sy_info& info, order_fr& order, double price_cent)
{
    ...
    if ((SPOT == info.type && info.long_short_flag == 0) || (SWAP == info.type && info.long_short_flag == 1)) { // 锟街伙拷锟斤拷锟洁，锟斤拷约锟斤拷锟絔1锟�1锟�77
        sum_mm = sum_mm + order.borrow * (*margin_mmr)[leverage];
    } else { // 锟街伙拷锟斤拷锟秸ｏ拷锟斤拷约锟斤拷锟斤拷1锟�1锟�77
    ...
    for (auto it : BnApi::BalMap_) {
        ...
        string sy = it.first;
        if (sy == "USDT" || sy == "USDC" || sy == "BUSD") {
            sum_mm = sum_mm + it.second.crossMarginBorrowed + (*margin_mmr)[leverage] * 1; // 锟杰革拷锟街伙拷维锟街憋拷证锟�1锟�1锟�77
        } else {
...
// flag 1 arb , 0 fr
//close arb_thresh/fr_thresh 锟斤拷锟斤拷锟斤拷锟斤拷为锟斤拷maker锟斤拷锟斤拷  maker锟斤拷taker 锟斤拷锟斤拷芨叨锟斤拷锟�(at most larger than taker)锟斤拷锟斤拷maker锟斤拷锟洁，锟斤拷锟斤拷maker锟斤拷taker锟斤拷锟斤拷要锟竭讹拷锟斤拷(at least large than taker)
...
//open fr_thresh 锟斤拷锟斤拷锟斤拷锟斤拷为锟斤拷maker锟斤拷锟斤拷, maker锟斤拷taker 锟斤拷锟斤拷要锟竭讹拷锟斤拷(at least larger than taker)锟斤拷锟斤拷maker锟斤拷锟洁，锟斤拷锟斤拷maker锟斤拷taker锟斤拷锟斤拷芨叨锟斤拷锟�(at least large than taker)
