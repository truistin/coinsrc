...
double StrategyFR::calc_future_uniMMR(sy_info& info, double qty)
{
    ...
    if (IS_DOUBLE_GREATER(IM, sum_equity)) {
        LOG_INFO << "�ֻ�+��Լ�ĳ�ʼ��֤�� > ��Ч��֤�𣬲������`1�1�77: " << IM << ", sum_equity: " << sum_equity;
        return 0;
    }
...
double StrategyFR::calc_predict_equity(sy_info& info, order_fr& order, double price_cent)
{
    ...
    if ((SPOT == info.type && info.long_short_flag == 0) || (SWAP == info.type && info.long_short_flag == 1)) { // �ֻ������1�1�77 ��Լ����
...
double StrategyFR::calc_predict_mm(sy_info& info, order_fr& order, double price_cent)
{
    ...
    if ((SPOT == info.type && info.long_short_flag == 0) || (SWAP == info.type && info.long_short_flag == 1)) { // �ֻ����࣬��Լ���]1�1�77
    ...
    for (auto it : BnApi::BalMap_) {
        ...
        string sy = it.first;
        if (sy == "USDT" || sy == "USDC" || sy == "BUSD") {
            sum_mm = sum_mm + it.second.crossMarginBorrowed + (*margin_mmr)[leverage] * 1; // �ܸ��ֻ�ά�ֱ�֤�1�1�77
        } else {
...
// flag 1 arb , 0 fr
//close arb_thresh/fr_thresh ��������Ϊ��maker����  maker��taker ����ܸ߶���(at most larger than taker)����maker���࣬����maker��taker����Ҫ�߶���(at least large than taker)
...
//open fr_thresh ��������Ϊ��maker����, maker��taker ����Ҫ�߶���(at least larger than taker)����maker���࣬����maker��taker����ܸ߶���(at least large than taker)
