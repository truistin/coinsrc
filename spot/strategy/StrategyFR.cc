...
double StrategyFR::calc_future_uniMMR(sy_info& info, double qty)
{
    ...
    if (IS_DOUBLE_GREATER(IM, sum_equity)) {
        LOG_INFO << "锟街伙拷+锟斤拷约锟侥筹拷始锟斤拷证锟斤拷 > 锟斤拷效锟斤拷证锟金，诧拷锟斤拷锟斤拷锟铰卄1锟�1锟�71锟�1锟�777: " << IM << ", sum_equity: " << sum_equity;
        return 0;
    }
...
double StrategyFR::calc_predict_equity(sy_info& info, order_fr& order, double price_cent)
{
    ...
    if ((SPOT == info.type && info.long_short_flag == 0) || (SWAP == info.type && info.long_short_flag == 1)) { // 锟街伙拷锟斤拷锟斤拷锟�1锟�71锟�1锟�71锟�1锟�777 锟斤拷约锟斤拷锟斤拷
    ...
    } else { // 锟街伙拷锟斤拷锟秸★拷1锟�1锟�71锟�1锟�777 锟斤拷约锟斤拷锟斤拷
...
double StrategyFR::calc_predict_mm(sy_info& info, order_fr& order, double price_cent)
{
    ...
    if ((SPOT == info.type && info.long_short_flag == 0) || (SWAP == info.type && info.long_short_flag == 1)) { // 锟街伙拷锟斤拷锟洁，锟斤拷约锟斤拷锟絔1锟�1锟�71锟�1锟�777
        sum_mm = sum_mm + order.borrow * (*margin_mmr)[leverage];
    } else { // 锟街伙拷锟斤拷锟秸ｏ拷锟斤拷约锟斤拷锟斤拷1锟�1锟�71锟�1锟�777
    ...
    for (auto it : BnApi::BalMap_) {
        ...
        string sy = it.first;
        if (sy == "USDT" || sy == "USDC" || sy == "BUSD") {
            sum_mm = sum_mm + it.second.crossMarginBorrowed + (*margin_mmr)[leverage] * 1; // 锟杰革拷锟街伙拷维锟街憋拷证锟�1锟�71锟�1锟�71锟�1锟�777
        } else {
...
void StrategyFR::hedge(StrategyInstrument *strategyInstrument)
{
    ...
    if (IS_DOUBLE_GREATER_EQUAL(delta_posi * sy1.mid_p, sy1.min_amount)) {
    ...
    } else if (IS_DOUBLE_LESS_EQUAL(abs(delta_posi * sy1.mid_p), sy1.min_amount)) {
        if ((sy1.make_taker_flag == 1) && (sy1.long_short_flag == 1) && IS_DOUBLE_LESS(sy1.real_pos, 0)) {
            order.orderType = ORDERTYPE_MARKET; // ?
            if (SWAP == sy2->type) 
                memcpy(order.Category, LINEAR.c_str(), min(uint16_t(CategoryLen), uint16_t(LINEAR.size())));
...
// flag 1 arb , 0 fr
//close arb_thresh/fr_thresh 锟斤拷锟斤拷锟斤拷锟斤拷为锟斤拷maker锟斤拷锟斤拷  maker锟斤拷taker 锟斤拷锟斤拷芨叨锟斤拷锟�1锟�7(at most larger than taker)锟斤拷锟斤拷maker锟斤拷锟洁，锟斤拷锟斤拷maker锟斤拷taker锟斤拷锟斤拷要锟竭讹拷锟斤拷(at least large than taker)
bool StrategyFR::ClosePosition(const InnerMarketData &marketData, sy_info& sy, int closeflag)
{
    ...
    sy_info* sy2 = sy.ref;
    if (IS_DOUBLE_GREATER(abs(-sy.real_pos - sy2->real_pos) * sy.avg_price, 3 * sy.fragment)) {
        LOG_INFO << "more than 3 * fragment";
        return;
    }
    if (sy.make_taker_flag == 1) {
        if ((sy.long_short_flag == 1) && IS_DOUBLE_LESS(sy.real_pos, 0)) {
            ...
            if (IS_DOUBLE_LESS(spread_rate, thresh)) {
                ...
                if (IS_DOUBLE_LESS(qty * sy.mid_price, sy.min_amount)) return
                SetOrderOptions order;
        ...
        if ((sy2->long_short_flag == 1) && IS_DOUBLE_LESS(sy2->real_pos, 0)) {
            ...
            if (IS_DOUBLE_LESS(spread_rate, thresh)) {
                ...
                if (IS_DOUBLE_LESS(qty * sy2.mid_price, sy2.min_amount)) return
...
//open fr_thresh 锟斤拷锟斤拷锟斤拷锟斤拷为锟斤拷maker锟斤拷锟斤拷, maker锟斤拷taker 锟斤拷锟斤拷要锟竭讹拷锟斤拷(at least larger than taker)锟斤拷锟斤拷maker锟斤拷锟洁，锟斤拷锟斤拷maker锟斤拷taker锟斤拷锟斤拷芨叨锟斤拷锟�1锟�7(at least large than taker)
void StrategyFR::OnRtnInnerMarketDataTradingLogic(const InnerMarketData &marketData, StrategyInstrument *strategyInstrument)
{
    ...
    if (IS_DOUBLE_GREATER(abs(-sy1.real_pos - sy2->real_pos) * sy1.avg_price, 3 * sy1.fragment)) {
        LOG_INFO << "more than 3 * fragment";
        return;
    }
    ...
    if (sy1.make_taker_flag == 1) {
        if (sy1.long_short_flag == 1) {
            ...
            if (IS_DOUBLE_GREATER(spread_rate, sy1.fr_open_thresh)) {
                ...
                if (IS_DOUBLE_LESS(qty * sy1.mid_price, sy1.min_amount)) return
