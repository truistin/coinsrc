...
void StrategyFR::initInstrument() {
...
int StrategyFR::getBuyPendingLen(SyInfo& sy) {
    int pendNum = 0;
...
int StrategyFR::getSellPendingLen(SyInfo& sy) {
    int pendNum = 0;
...
int StrategyFR::getIocOrdPendingLen(SyInfo& sy) {
    int pendNum = 0;
    for (auto it : (*sy.strategyInstrument->sellOrders())) {
        for (auto iter : it.second->OrderList) {
...
double StrategyFR::get_tol_delay() {
...
int StrategyFR::getPoPriceLevel(OrderByPriceMap*  priceMap) {
    int level = 0;
...
// GTX pending orders
double StrategyFR::po_best_price(OrderByPriceMap*  priceMap, int side) {
    if (side == 0) {
        if (priceMap->size() != 0) {
...
bool StrategyFR::vaildPrice(SyInfo& sy) {
...
void StrategyFR::over_max_delta_limit() {
...
// taker_action maker_action only exchange2 ,hedge only exchange1.
void StrategyFR::hedge() {
    // uint64_t now_ns= CURR_NSTIME_POINT;
    // if (now_ns - last_hedge_time < hedge_time_gap * 1e9) return;
    // LOG_INFO << "StrategyFR::hedge(): " << delta_pos_notional << ", hedge_pos_thresh2: " << hedge_pos_thresh2;
...
void StrategyFR::log_delay(string s, string exch)
{
    uint64_t now_ns = CURR_NSTIME_POINT;
